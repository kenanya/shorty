...
func TestGetURLStat(t *testing.T) {
	...
	t.Run("positive case get url stat", func(t *testing.T) {

		var (
			e          = echo.New()
			resPayload = flowV1.ResponseGetURLStat{}
		)
		...
		req := httptest.NewRequest(http.MethodGet, "localhost:9701/v1/0ZzHBW/stats", nil)
		...
		// Assertions
		if assert.NoError(t, handler.GetURLStatByShortCode(ctx)) {
	...
	t.Run("negative case get url stat", func(t *testing.T) {
